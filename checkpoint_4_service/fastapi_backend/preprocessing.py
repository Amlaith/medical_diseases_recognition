...
PRETRAINED_PCA_PATH = "models/pca.pkl"
with open(PRETRAINED_PCA_PATH, 'rb') as pretrained_PCA:
    pca = pickle.load(pretrained_PCA)
...
def create_default_dataset(seed: int = 73) -> tuple[np.array, np.array]:
    ...
    sampled_class_1 = class_1.sample(NUM_IMAGES_PER_CLASS, random_state=seed)
    sampled_class_0 = class_0.sample(NUM_IMAGES_PER_CLASS, random_state=seed)
...
def get_scores(model, dataset, needs_fit=True) -> tuple[Any, float, float, np.array, np.array, float]:
    ...
    roc_auc = float(metrics.roc_auc_score(y_test, pred_proba))
    ...
    return model, acc, f2, list(fpr), list(tpr), roc_auc